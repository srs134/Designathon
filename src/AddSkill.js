...
import './AddSkill.css'; // Import the CSS file
...
const AddSkill = () => {
  ...
  return (
    <div className={`add-skill-app ${isRecording ? 'recording' : ''}`}>
      <div className="add-skill-timer">
        <span role="img" aria-label="clock">⏱️</span> {`Time: ${time}s`}
      </div>
      
      <div className="add-skill-buttons">
        <button onClick={isRecording ? stopRecognition : startRecognition} className="add-skill-talk-button">
          <span role="img" aria-label="microphone">{isRecording ? '🛑' : '🎙️'}</span> {isRecording ? 'Stop' : 'Talk'}
        </button>
      </div>
      
      <div className="add-skill-input-container">
        <textarea className="add-skill-input-box" readOnly placeholder="Recognized words will appear here..." value={transcript} />
      </div>
